...
class Sensor:
    ...
    def in_fov(self, x):
        ...
        pos = np.ones((4, 1))
        pos[0:3] = x[0:3]
        # from vehicle coord to sensor coord
        pos = self.veh_to_sens * pos

        if pos[0] > 0:
            alpha = np.arctan(pos[1] / pos[0])
    ...
    def get_hx(self, x):
        ...
        if self.name == 'lidar':
            ...
            pos_veh = np.ones((4, 1))
            pos_veh[0:3] = x[0:3]
            pos_camera = self.veh_to_sens * pos_veh

            if pos_camera[0] == 0:
                assert "Error cannot divide by 0. pos_camera[0] equals 0."

            h_x = np.zeros((2, 1))
            h_x[0][0] = self.c_i - self.f_i * (pos_camera[1] / pos_camera[0])
            h_x[1][0] = self.c_j - self.f_j * (pos_camera[2] / pos_camera[0])
