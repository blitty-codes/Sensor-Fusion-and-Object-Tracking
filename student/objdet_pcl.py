...
def show_pcl(pcl):
    ...
    vis.create_window("Open 3D lidar - ID_S1_EX2", width=800, height=600)
    vis.register_key_callback(262, callback_func=callback)
...
def show_range_image(frame, lidar_name):
    ...
    ri_range = ri[:, :, 0]
    ri_range = ri_range * 255 / (np.amax(ri_range) - np.amin(ri_range)).astype(np.uint8)
    ...
    ri_range = ri_range[:, ri_center - deg45:ri_center + deg45]
...
def bev_from_pcl(lidar_pcl, configs, vis=True):
    ...
    lidar_pcl_cp = np.copy(lidar_pcl)
    lidar_pcl_cp[:, 0] = np.int_(np.floor(lidar_pcl_cp[:, 0] / discr))

    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_pcl_cp[:, 1] = np.int_(np.floor(lidar_pcl_cp[:, 1] / discr) + (configs.bev_width + 1) / 2)
    lidar_pcl_cp[:, 1] = np.abs(lidar_pcl_cp[:, 1])
    ...
    lidar_pcl_cp[lidar_pcl_cp[:, 3] > 1.0, 3] = 1.0
    indices = np.lexsort((-lidar_pcl_cp[:, 3], lidar_pcl_cp[:, 2], lidar_pcl_cp[:, 1]))
    lidar_pcl_top = lidar_pcl_cp[indices]

    ## step 3 : extract all points with identical x and y such that only the top-most z-coordinate is kept (use numpy.unique)
    ##          also, store the number of points per x,y-cell in a variable named "counts" for use in the next task
    _, indices, counts = np.unique(lidar_pcl_top[:, 0:2], axis=0, return_index=True, return_counts=True)
