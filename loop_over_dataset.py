...
model = 'darknet'
exec_detection, exec_data, exec_tracking, exec_visualization = [], [], [], []
...
data_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'dataset',
                             data_filename)  # adjustable path in case this script is called from another working directory
results_fullpath = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results')# os.path.join(os.path.dirname(os.path.realpath(__file__)), 'results\\' + model + '\\results_sequence_' + sequence + '_' + model)
    ...
    try:
        ...
        if 'perform_tracking' in exec_list:
            ...
            for detection in detections:
                ...
                if detection[1] > configs_det.lim_x[0] and detection[1] < configs_det.lim_x[1] and detection[2] > \
                        configs_det.lim_y[0] and detection[2] < configs_det.lim_y[1]:
